...
function check_init()
{
    if [ ! -f $CCC_CONFIG_FILE ];then
          perror "file $CCC_CONFIG_FILE is not exist"
    fi
}
...
function check_cc_controller_process()
{
    ...
    exit_str_null "cc_controll is not running" $pid
    pcheck "process_id of cc_controller is $pid, result is OK"
}

function check_ovs_connected()
{
    result=$(ovs-vsctl show | head -n 10 | grep is_connected | cut -d":" -f2 | sed -e 's/\ //g')
    if [ "$result" != "true" ];then
...
function check_ovs_fail_mode()
{
    ...
    result=$(ovs-vsctl show | head -n 10 | grep fail_mode|sed -e 's/\ //g' | cut -d":" -f2)
