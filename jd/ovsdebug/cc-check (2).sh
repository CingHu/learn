...
JVIRTRC="source /root/jvirtrc"

DCMSG=`echo ${SOURCENAME[*]}`
...
while getopts "d:o:f:v:p:c:t:hs" arg
do
        case $arg in
             d)
                DCNAME=$OPTARG
...
function check_server_new_compute()
{
    local vmid=$1
    pcheck "================ vm $vmid info ============"
    newsshcmd jvirt instance-show $vmid -a > "$PATHINFO/newvm"
...
function del_check_flow()
{
    ...
    if [ "$isdrop" != "" ];then
        ...
        cat "$PATHINFO/flowresult"
    fi
    sshexec ''$cmd' flow_pat_enable '${flowid}' 0'
    sshexec ''$cmd' flow_pat_del '${flowid}''
}
