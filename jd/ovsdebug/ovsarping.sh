...
function checkRcvARPPkt() {
    ...
    Info "$TCPDUMP -w $PORTID.iLoop.dmp -c 1  arp and ether src host $SMAC and ether dst host $SMAC and ether[20:2] == 0x0002"
    for ((iLoop=0; iLoop<$RCVLOOPNUM; iLoop++))
    do
        {
            $TCPDUMP -w $PORTID.$iLoop.dmp -c 1  arp and ether src host $SMAC and ether dst host $SMAC and 'ether[20:2] == 0x0002' -q 2> /dev/null &
            pid=$!
