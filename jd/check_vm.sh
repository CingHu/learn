...
function get_linux_vm_route(){
    ...
    real_result=$(echo ${result}| sed -e 's/\"//g' -e 's/\ //g' -e 's/[0-9]//g')
    if [ "${real_result}" == "" ];then
        perror "[ip] get route info failed, ${result}"
        exit 1 
    fi
    if [ ${sanity_result} != ${real_result} ];then
        perror "[ip] get route info failed, ${result}"
...
function get_linux_vm_dhcp_service(){
    ...
    real_result=$(echo ${result}| sed -e 's/\"//g' -e 's/\ //g' -e 's/[0-9]//g')
    if [ "${real_result}" == "" ];then
        perror "[ip] get dhcp info failed, ${result}"
        exit 1 
    fi
    if [ ${sanity_result} != ${real_result} ];then
        perror "[ip] get dhcp info failed, ${result}"
...
function get_linux_vm_iptables(){
    ...
    real_result=$(echo ${result}| sed -e 's/\"//g' -e 's/\ //g' -e 's/[0-9]//g')
    if [ "${real_result}" == "" ];then
        perror "[ip] get iptables info failed, ${result}"
        exit 1 
    fi
    if [ ${sanity_result} != ${real_result} ];then
        perror "[ip] get iptables info failed, ${result}"
...
function get_linux_cmd_info(){
    local icmd=$1
    local result=""
    local real_result=""
    local cmd='{"execute": "guest-exec", "arguments": {"path": "/bin/sh",  "capture-output": true, "arg": ["-c", '$icmd']}}'
    local sanity_result="{return:{pid:}}"

    result=$(${VMCMD} ${cmd})
    real_result=$(echo ${result}| sed -e 's/\"//g' -e 's/\ //g' -e 's/[0-9]//g')
    if [ "${real_result}" == "" ];then
        perror "[ip] get cmd ${icmd} info failed, ${result}"
        exit 1 
    fi
    if [ ${sanity_result} != ${real_result} ];then
        perror "[ip] get cmd ${icmd} info failed, ${result}"
...
function get_win_vm_firewall(){
    ...
    real_result=$(echo ${result}| sed -e 's/\"//g' -e 's/\ //g' -e 's/[0-9]//g')
    if [ "${real_result}" == "" ];then
        perror "[ip] get firewall info failed, ${result}"
        exit 1 
    fi
    if [ ${sanity_result} != ${real_result} ];then
        perror "[ip] get firewall info failed, ${result}"
...
function get_win_cmd_info(){
    local result=""
    local icmd=$1
    local real_result=""
    local cmd='{"execute": "guest-exec", "arguments": {"path": "C:\\Windows\\system32\\cmd.exe", "capture-output": true, "arg": ["/C", "'${icmd}'> COM1"]}}'
    local sanity_result="{return:{pid:}}"
    ...
    get_win_info $CMDINFO
}

function check_win_cmd(){
    if [ "${CMD}" == "" ];then
        return
    fi
    get_win_cmd_info ${CMD}
}
