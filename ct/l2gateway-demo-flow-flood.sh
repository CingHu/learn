...
ovs-ofctl add-flow $TUNBRIDGE  "table=0,  priority=1 actions=drop"
...
ovs-ofctl add-flow $TUNBRIDGE  "table=2,  priority=100,dl_dst=$gwmac actions=resubmit(,3)"
ovs-ofctl add-flow $TUNBRIDGE  "table=2,  priority=1 actions=resubmit(,3)"

#flood
ovs-ofctl add-flow $TUNBRIDGE  "table=3,  priority=100,tun_id=$vni  actions=set_field:$tunnelip->tun_dst,resubmit(,4)"
ovs-ofctl add-flow $TUNBRIDGE  "table=3,  priority=1 actions=drop"

#output
ovs-ofctl add-flow $TUNBRIDGE  "table=4,  priority=100 actions=output:vxlan0"

ovs-ofctl add-flow $BASEBRIDGE  "table=0, priority=100, in_port=bond1,dl_vlan=$vlan actions=output:${patchp}"
...
$ADDFLOW "table=10, priority=1 actions=resubmit(,30)"
...
$ADDFLOW "table=20, priority=100 actions=output:$patchtunl"
$ADDFLOW "table=20, priority=1 actions=mod_vlan_vid:30,resubmit(,100)"
