...
function help() {
    ...
    echo "vxlan.icmp6.type             capture the vxlan packets whose overlay protocol is icmp6 type, only ipv6"
    echo "vxlan.nd                     capture the vxlan packets whose overlay protocol is nd, include ns, na, rs, ra, redirect, only ipv6"
    ...
    echo " ipv6 example: ./$0 -i bond1 vxlan.icmp6.type 133"
...
function replace_pro() {
    ...
    if [ $proto ]; then
        if [ $proto = "vxlan.tcp" ]; then
            ...
            local vpn=" udp[36]=0x3a and udp[28:2]=0x86dd and (udp[70]=0x87 or udp[70]=0x88 or udp[70]=0x85 or udp[70]=0x86 or udp[70]=0x89)"
            ARGS=${ARGS/$proto/$vpn}
        fi
    fi
    return
...
function replace_icmp6() {
    local np=" udp[28:2] = 0x86dd and udp[36] = 0x3a"
    ARGS=${ARGS/vxlan.icmp6/$np}
}

function replace_icmp6_type() {
    local v6type=$1
    local np=" udp[70] = "${v6type}
...
#        echo $param $value
        if [ $param = "vxlan.smac" ] && [ $value ]; then
            ...
            replace_icmp6 $param
        elif [ $param = "vxlan.icmp6.type" ]; then
            replace_icmp6_type $value
        fi
    fi
    return
}
