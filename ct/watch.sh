...
((i+=1))
if [[ $1 == $i ]];then
watch -n 1 -d 'ovs-ofctl dump-flows br0 -O openflow13 | grep -v "OFPST_FLOW"| sed -e "/NXST_FLOW/d" -e "s/\(duration\)=[^,]*,//g" -e "s/send_flow_rem//g" | grep -v  "n_packets=0" |  grep -E "n_packets=[0-9]{4}"'
fi
