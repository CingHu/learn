...
ovs-ofctl add-flow $TUNBRIDGE  "table=0,  priority=1 actions=resubmit(,1)"
ovs-ofctl add-flow $TUNBRIDGE  "table=1,  priority=100,dl_dst=$vmmac  actions=set_field:$vni->tun_id, set_field:$localip->tun_src,set_field:$tunnelip->tun_dst,output:vxlan0"
ovs-ofctl add-flow $TUNBRIDGE  "table=1,  priority=100,dl_dst=$gwmac,arp  actions=set_field:$vni->tun_id, set_field:$localip->tun_src,resubmit(,2)"
ovs-ofctl add-flow $TUNBRIDGE  "table=1,  priority=1 actions=drop"
ovs-ofctl add-flow $TUNBRIDGE  "table=2,  priority=100,tun_id=$vni  actions=set_field:$tunnelip->tun_dst,output:vxlan0"
ovs-ofctl add-flow $TUNBRIDGE  "table=2,  priority=1 actions=drop"
...
ovs-ofctl add-flow $BASEBRIDGE  "table=0, priority=100, in_port=bond1,dl_vlan=$vlan actions=strip_vlan,output:${patchp}"
ovs-ofctl add-flow $BASEBRIDGE  "table=0, priority=100,in_port=$patchp actions=mod_vlan_vid:$vlan,output:bond1"
ovs-ofctl add-flow $BASEBRIDGE  "table=0, priority=1,in_port=$patchp actions=drop"
...
$ADDFLOW "table=10, priority=100,ip, nw_dst=$vmip  actions=mod_dl_dst:$vmmac,resubmit(,20)"
$ADDFLOW "table=10, priority=100,ip, nw_dst=$gwip  actions=mod_dl_dst:$gwmac,resubmit(,20)"
$ADDFLOW "table=10, priority=1 actions=resubmit(,20)"

$ADDFLOW "table=15,priority=100,arp,arp_op=1, arp_tpa=$vmip actions=load:0x2->NXM_OF_ARP_OP[],move:NXM_NX_ARP_SHA[]->NXM_NX_ARP_THA[],move:NXM_OF_ARP_SPA[]->NXM_OF_ARP_TPA[],load:$hostmachex->NXM_NX_ARP_SHA[],load:$vmiphex->NXM_OF_ARP_SPA[],move:NXM_OF_ETH_SRC[]->NXM_OF_ETH_DST[],mod_dl_src:$hostmac,IN_PORT"
$ADDFLOW "table=15,priority=100,arp,arp_op=2, arp_tpa=$vmip actions=load:$vmmachex->NXM_NX_ARP_THA[], mod_dl_dst:$vmmac, resubmit(,20)"
$ADDFLOW "table=15,priority=100,arp,arp_op=2, arp_tpa=$gwip actions=load:$gwmachex->NXM_NX_ARP_THA[], mod_dl_dst:$gwmac, resubmit(,20)"

$ADDFLOW "table=20, priority=1 actions=strip_vlan,output:$patchtunl"

$ADDFLOW "table=40, priority=100,arp actions=resubmit(,45)"
$ADDFLOW "table=40, priority=10,ip actions=mod_dl_src:$hostmac,output:$patchl"
$ADDFLOW "table=40, priority=1 actions=mod_vlan_vid:40,resubmit(,100)"


$ADDFLOW "table=45,priority=100,arp,arp_op=1 actions=load:$hostmachex->NXM_NX_ARP_SHA[], mod_dl_src:$hostmac, output:$patchl"
