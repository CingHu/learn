		...
		if (nlink > 0)
		  {
		    ...
		    rv =
		      rte_eth_bond_mac_address_set (dev->port_id,
						    (struct ether_addr *)
							    addr);
            if (rv < 0)
		      VLOG_ERR ("Failed to set MAC address");
          }
      }
#endif
    ...
    OVS_REQUIRES(dpdk_mutex)
{
     ...
     * use 'SOCKET0'. */
    if ((type == DPDK_DEV_ETH) || (type == DPDK_DEV_ETH_BOND)){
    ...
    for (i = 0; i < nports; i++)
      {
        ...
        VLOG_WARN(" dev_info.driver_name = %s", dev_info.driver_name);

        if (strncmp (dev_info.driver_name, "rte_bond_pmd", 12) == 0)
	      {
            VLOG_WARN(" rte_bond_pmd.driver_name = %s", dev_info.driver_name);
          }
      }
 #endif 
...
netdev_dpdk_register(void)
{
    ...
    netdev_register_provider(&dpdk_bond_class);
